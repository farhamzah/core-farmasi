...
class IssueKppspaApiClientCommandTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_dry_run_does_not_create_core_api_client_or_secret(): void
    {
        ...
        $exitCode = Artisan::call('core:issue-kppspa-api-client');
    ...
    public function test_apply_creates_active_kppspa_client_with_required_abilities(): void
    {
        $this->makeApplication();
        $this->makeRoles();

        $exitCode = Artisan::call('core:issue-kppspa-api-client', ['--apply' => true]);
        $output = Artisan::output();

        $this->assertSame(0, $exitCode);

        $client = CoreApiClient::query()->where('app_code', 'kppspa-farmasi')->firstOrFail();

        $this->assertTrue($client->is_active);
        $this->assertNull($client->revoked_at);
        $this->assertNotEmpty($client->secret_hash);
        $this->assertSame(app(AppConnectionReadinessService::class)->requiredAbilities('kppspa-farmasi'), $client->abilities);
    ...
    public function test_apply_again_does_not_create_duplicate_client(): void
    {
        ...
        Artisan::call('core:issue-kppspa-api-client', ['--apply' => true]);
        Artisan::call('core:issue-kppspa-api-client', ['--apply' => true]);
        $output = Artisan::output();

        $this->assertSame(1, CoreApiClient::query()->where('app_code', 'kppspa-farmasi')->count());
    ...
    public function test_show_env_template_uses_placeholders_only(): void
    {
        ...
        $exitCode = Artisan::call('core:issue-kppspa-api-client', ['--show-env-template' => true]);
        $output = Artisan::output();

        $this->assertSame(0, $exitCode);
        $this->assertStringContainsString('CORE_FARMASI_CLIENT_ID=<client_id>', $output);
        $this->assertStringContainsString('CORE_FARMASI_CLIENT_SECRET=<copy-secret-once>', $output);
        $this->assertStringNotContainsString('Client Secret (shown once):', $output);
        $this->assertSame(0, CoreApiClient::query()->count());
    }

    public function test_command_refuses_when_kppspa_application_missing_or_inactive(): void
    {
        $missingExitCode = Artisan::call('core:issue-kppspa-api-client', ['--apply' => true]);
        $this->assertSame(1, $missingExitCode);

        $this->makeApplication(['is_active' => false]);

        $inactiveExitCode = Artisan::call('core:issue-kppspa-api-client', ['--apply' => true]);
        $output = Artisan::output();
    ...
    public function test_readiness_after_apply_has_complete_api_client(): void
    {
        ...
        $summary = app(AppConnectionReadinessService::class)->readinessSummary('kppspa-farmasi');
    ...
    {
        $application = CoreApplication::query()->where('app_code', 'kppspa-farmasi')->first();

        foreach (app(AppConnectionReadinessService::class)->requiredRoleSlugs('kppspa-farmasi') as $index => $roleSlug) {
